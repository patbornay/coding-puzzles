...
const mergeSort15 = (nums: number[]): number[] => {
    if (nums.length <= 1) return nums;

    const mid = Math.floor(nums.length / 2);
    const left = mergeSort15(nums.slice(0, mid));
    const right = mergeSort15(nums.slice(mid));
    return merge15(left, right);
}

const merge15 = (left: number[], right: number[]): number[] => {
    let result: number[] = [], i = 0, j = 0;
    while (i < left.length && j < right.length) {
        if (left[i] < right[j]) result.push(left[i++]);
        else result.push(right[j++]);
...
// rep 14 31/5/25
const mergeSort14 = (nums: number[]): number[] => {
    if (nums.length <= 1) return nums;

    const mid = Math.floor(nums.length / 2);
    const left = mergeSort14(nums.slice(0,mid));
    const right = mergeSort14(nums.slice(mid));

    return merge14(left, right);
}

const merge14 = (left: number[], right: number[]) => {
    const result: number[] = [];
    let i = 0, j = 0;
    while (i < left.length && j < right.length) {
        if (left[i] < right[j]) result.push(left[i++]);
...
// rep 13 30/5/25
const mergeSort13 = (nums: number[]): number[] => {
    if (nums.length <= 1) return nums;

    const mid = Math.floor(nums.length / 2);
    const left = mergeSort13(nums.slice(0,mid));
    const right = mergeSort12(nums.slice(mid));
    return merge13(left, right);
}

const merge13 = (left: number[], right: number[]) => {
    const result: number[] = []
    let i = 0, j = 0;
    while (i < left.length && j < right.length) {
        if (left[i] < right[j]) result.push(left[i++]);
...
// rep 12 1/5/25
const mergeSort12 = (nums: number[]): number[] => {
    // base case
    if (nums.length <= 1) return nums;

    // recursively setup mid and left and right
    const mid = Math.floor(nums.length / 2);
    const left = mergeSort12(nums.slice(0,mid));
    const right = mergeSort12(nums.slice(mid));

    //merge the result
    return merge12(left,right);
}

const merge12 = (left: number[], right: number[]): number[] => {
    // while loop over left and right while pushing to results 
    let result: number[] = [], i = 0, j = 0;
    // return the remaining elements from left and right concated to the end of results
    while (i < left.length && j < right.length) {
        if (left[i] < right[j]) result.push(left[i++]);
        else result.push(right[j++]);
    }
...
// rep 11 24/4/25 
const mergeSort11 = (nums: number[]): number[] => {
    if (nums.length <= 1) return nums;

    const mid = Math.floor(nums.length / 2);
    const left = mergeSort11(nums.slice(0, mid));
    const right = mergeSort11(nums.slice(mid));

    return merge11(left, right);
}

const merge11 = (left: number[], right: number[]): number[] => {
    let result: number[] = [], i = 0, j = 0;
...
// rep 10 12/4/24
const mergeSort10 = (nums: number[]) => {
    if (nums.length <= 1) return nums;

    const mid = Math.floor(nums.length / 2);
    const left: number[] = mergeSort10(nums.slice(0,mid));
    const right: number[] = mergeSort10(nums.slice(mid));

    return merge(left, right);
}

const merge10 = (left: number[], right: number[]): number[] => {
    let results: number[] = [], i: number = 0, j: number = 0;
...
// rep 8 31/03/25
const mergeSort9 = (nums: number[]) => {
    if (nums.length <= 1) return nums;

    const mid = Math.floor(nums.length / 2);
    const left = mergeSort9(nums.slice(0, mid));
    const right = mergeSort9(nums.slice(mid));

    return merge9(left, right);
}

const merge9 = (left: number[], right: number[]) => {
...
// rep 8 28/03/25
const mergeSort8 = (nums: number[]) => {
    if (nums.length <= 1) return nums;

    const mid = Math.floor(nums.length / 2);
    const left = mergeSort8(nums.slice(0, mid));
    const right = mergeSort8(nums.slice(mid));

    return merge8(left, right);
}

const merge8 = (left: number[], right: number[]): number[] => {
    let results: number [] = [], i = 0, j = 0;

    while (i < left.length && j < right.length) {
        if (left[i] < right[j]) results.push[i++];
        else results.push(right[j++]);
    }
    return results.concat(left.slice(i).concat(right.slice(j)));
}
...
// rep 7 24/03/25
const mergeSort7 = (nums: number[]) => {
    if (nums.length <= 1) return nums;

    const mid = Math.floor(nums.length / 2);
    const left = mergeSort7(nums.slice(0, mid));
    const right = mergeSort7(nums.slice(mid));

    return merge7(left, right);
}

const merge7 = (left: number[], right: number[]) => {
    let results: number[] = [], i = 0, j = 0;
